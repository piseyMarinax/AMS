...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<span>{{ trans('template.list-chart_account') }}</span>
		</li>
	</ul>
</div>
<br/>
		...
		<div class="portlet light bordered">			
			<div class="portlet-title">
				<div class="caption font-green">
					...
					<span class="caption-subject bold uppercase">{{ trans('template.list-chart_account') }}</span>
				</div>
				<div class="actions">
					<a title="{{ trans('template.add_new') }}" class="btn btn-circle btn-icon-only btn-default" href="{{ url('/accounting/chart_account/chart_accounting/add') }}">
				...
				<table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="table-typeaccounting">
					<thead>
						<tr class="bg-primary">
							...
							<th class="text-center min-phone-l">{{ trans('template.type_of_accounting') }}</th>
...
<script type="text/javascript">	
	$(function(){
		$('#table-typeaccounting').DataTable({
			processing: true,
			serverSide: true,
			ajax: '{{url("/accounting/chart_account/chart_accounting/list")}}',
			columns: [
				{data: 'id'},
				{data: 'ac_code'},
				{data: 'ac_name'},
	...
	function onDelete(id){
		...
		App.doDelete("{{ trans('template.delete').' '.trans('template.type_of_accounting') }}","{{trans('template.message_delete').' '.trans('template.type_of_accounting').' ?'}}",'/accounting/chart_account/chart_accounting/delete',id,'post',_token);
	}
</script>
@endsection()