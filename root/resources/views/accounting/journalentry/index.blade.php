...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<span>{{ trans('template.journal_entry') }}</span>
		</li>
	</ul>
</div>
<br/>
		...
		<div class="portlet light bordered">			
			<div class="portlet-title">
				<div class="caption font-green">
					...
					<span class="caption-subject bold uppercase">{{ trans('template.journal_entry') }}</span>
				</div>
				<div class="actions">
					<a title="{{ trans('template.add_new') }}" class="btn btn-circle btn-icon-only btn-default" href="{{ url('accounting/journal_entry/add') }}">
			...
			<div class="portlet-body">
				...
				@endif
				<table class="table table-striped table-bordered" width="100%" id="table-journal_entry">
					<thead>
						<tr class="bg-primary">
				...
				<script id="details-template" type="text/x-handlebars-template">
			        <table class="table details-table" id="journal_entry-@{{id}}">
			            <thead>
			                <tr>
...
<script type="text/javascript">	
	...
	var table =$('#table-journal_entry').DataTable({
		processing: true,
		serverSide: true,
		ajax: '{{url("/accounting/journal_entry/list")}}',
		columns: [
			{
	...
	$('#table-journal_entry tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = table.row(tr);
        var tableId = 'journal_entry-' + row.data().id;       
        if (row.child.isShown()){
            ...
            $('#journal_entry-'+row.data().id+'_length').addClass('col-xs-12 col-md-6');
        }
    });
    function initTable(tableId,id){
        ...
        $('#'+tableId).DataTable({
            ...
            ajax:'{{url("")}}/accounting/journal_entry/getDetailData/'+id,
            columns:[
	...
	function onDelete(id){
		...
		App.doDelete("{{ trans('template.delete').' '.trans('template.journal_entry') }}","{{trans('template.message_delete').' '.trans('template.journal_entry').' ?'}}",'/accounting/journal_entry/delete',id,'post',_token);
	}
</script>
@endsection()