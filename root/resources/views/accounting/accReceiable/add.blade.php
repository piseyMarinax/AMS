...
@section('content')
<style>
	#tableBalance tr td{padding: 2px 10px !important;}
	#tableBalance_paginate .pagination>li>a{padding: 5px 12px !important;}
	#tableBalance_wrapper .table-scrollable{ margin:0 !important;}
</style>
 <div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="{{url('accounting/account_receivable/index')}}">​{{ trans('template.account_receivable') }}</a>
			<i class="fa fa-circle"></i>
		</li>
		<li>
		...
		<div class="portlet light portlet-fit portlet-form bordered">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green sbold uppercase">{{ trans('template.account_receivable') }}</a></span>
				</div>
			</div>
			<div class="portlet-body form-body">
				<form class="form-horizontal formaccount_receivable" method="POST" action="{{url('accounting/account_receivable/insert')}}" id="form_validation">
					...
					<div class="form-body">
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-8">
							...
							<div class="form-group{{ $errors->has('account_receivable_date') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-3">{{trans('template.date')}}
								<span class="required"> * </span>
								</label>			                    
								<div class="col-md-9">
									<div class="input-icon right">
										<input class="form-control datepicker" type="text" name="account_receivable_date" id="account_receivable_date" placeholder="{{trans('template.date')}} - day-Month-Year" >
									</div>
									@if($errors->has('account_receivable_date'))
	                                        <span style="color:red">{!!$errors->first('account_receivable_date')!!} </span>
									@endif	
								</div>
							</div>
							...
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-3">{{trans('template.chart_account')}}
									<span class="required"> * </span>
								</label>			                    
								<div class="col-md-9">
									<div class="input-icon right">
							...
							<div class="form-group{{ $errors->has('class') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-3">{{trans('template.class')}}
								</label>			                    
								<div class="col-md-9">
									<div class="input-icon right">
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-4">
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-6" style="color: green; font-size: 18px;">{{trans('template.balance')}} :</label>
								<label class="control-label col-md-6" id="MainBalance" style="color:red;font-size:20px;"> $ 0.00 </label>			    
								<table class="table table-striped table-bordered table-hover dt-responsive" id="tableBalance" width="100%" >
									<thead>
										<tr style="text-align: center;" class="btn-success">
											<td >{{trans('template.date')}}</td>	
						...
						<table class="table table-striped table-bordered table-hover dt-responsive" id="tableaccount_receivable" width="100%" >
							<thead>
								<tr style="text-align: center;" class="btn-success">
					...
					<div class="form-actions">
						<div class="row">
							<div class="col-md-offset-4 col-md-6">
								<button type="submit"  name="btnSave" value="save" class="btn green" id="Saveaccount_receivable"> <i class="fa fa-save"> </i> <strong> {{trans('template.save')}}</strong></button>
...
$(document).ready(function() {
	table_account_receivable = $('#tableaccount_receivable').DataTable({
		"filter":   false,
		"paging":   false,
	    "ordering": false,
	    "info":     false
	});	
	tableBalance = $('#tableBalance').DataTable({
		"filter":   false,
		//"paging":   false,
	    "ordering": false,
	    "info":     false,
	...
	$('#tableBalance_length').css('display','none');
});
var counter = 1;
$('.AddNew').click(function(){	
	table_account_receivable.row.add( [
...
function deleteRow(id){
	table_account_receivable.row($('#'+id).parents('tr')).remove().draw( false );
	CalculateDataTable();
}
...
function CalculateDataTable(){
	var amount = 0;
	var amount_tax = 0;
	$('#tableaccount_receivable tbody tr').each(function() {
...
$('#Saveaccount_receivable').click(function(){
	if(onSubmit()){
	}else{
...
function onSubmit(){
	...
	var account_receivable_date = $('#account_receivable_date').val();
	var user_id = $('#user_id').val();
	var chart_account = $('#chart_account').val();
	var chart_account_name = $('.chart_account_name').val();
	var enter_price  = $('.enter_price ').val();
	if(referent_no=="" || account_receivable_date=="" || user_id==0 || chart_account==0 || chart_account_name==0 || enter_price =="" || chart_account==0 || chart_account_name==0 || enter_price =="" ){
		if(referent_no !=""){
			$('#referent_no').css("border", "1px solid #ccc");
		}else{
			$('#referent_no').css("border", "1px solid red");
		}
		if(account_receivable_date !=""){
			$('#account_receivable_date').css("border", "1px solid #ccc");
		}else{
			$('#account_receivable_date').css("border", "1px solid red");
		}
		if(user_id !=0){
