	...
	<div class="col-md-12">
		<div class="portlet light portlet-fit portlet-form bordered">
			...
			<div class="portlet-body form-body">
				<form class="form-horizontal formIncome" method="POST" action="{{url('accounting/income/update')}}" id="form_validation">
					<input type="hidden" name="_token" value="{{ csrf_token() }}" />
					<input type="hidden" name="update_id" value="{{$income->jn_id}}" />
					<div class="form-body">
						@if(count($errors))
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-6">
							<div class="form-group{{ $errors->has('referent_no') ? ' has-error' : '' }}  margin-top-20">
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<i class="fa"></i>
										<input type="text" value="{{ $income->code }}" class="form-control" name="referent_no" id="referent_no" placeholder="{{trans('template.referent_no')}}" /> </div>
							...
							<div class="form-group{{ $errors->has('income_date') ? ' has-error' : '' }}  margin-top-20">
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<input class="form-control datepicker" type="text" value="<?php echo date('d-m-Y',strtotime($income->income_date)); ?>" name="income_date" id="income_date" placeholder="{{trans('template.date')}} - day-Month-Year" >
									</div>
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<select name="user_id" class="form-control select2" id="user_id" style="width: 100%;">
											...
											{{\App\Helpers\Helpers::getUsers($income->user_id)}}
										</select>
									</div>
									@if($errors->has('name'))
								...
								<div class="col-md-8">
									<div class="input-icon right">
										<select name="chart_account" class="form-control select2" id="chart_account">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getChartAccountID([1,4,5],$income->acc_id)}}
										</select>
									</div>
								...
								<div class="col-md-8">
									<div class="input-icon right">
										<select name="class" class="form-control select2 value_class" id="AddClass">
											...
											{{\App\Helpers\Helpers::getClasses(0,0,$income->class)}}
										</select>
									</div>
									@if($errors->has('class'))
								...
								<tr>
									...
									<td style="width: 30%;">
										<select name="arr_chart_account[]" class="form-control select2 chart_account_name">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getAllChart(0,0,$data->jn_ac_type,4)}}
										</select>
									</td>
									<td
										<input type="text" value="<?php echo $data->jn_crb;?>" class="form-control enter_price enter_price<?php echo $key;?>" value="" onkeyup="EnterQty(this)" name="amount[]" placeholder="{{trans('template.amount')}}">
									</td>
									<td>
					...
					<div class="form-actions">
						<div class="row">
							<div class="col-md-offset-4 col-md-6">
								<button type="submit"  name="btnSave" value="save" class="btn green" id="SaveIncome"> <i class="fa fa-save"> </i> <strong> {{trans('template.update')}}</strong></button>
								<a href="{{url('accounting/income/index')}}"><button type="button" id="btnClose" class="btn btn-danger"> <i class="fa fa-times-circle"> </i> <strong> {{trans('template.cancel')}}</strong></button> </a>
							</div>
						</div>
					</div>
				</form>
