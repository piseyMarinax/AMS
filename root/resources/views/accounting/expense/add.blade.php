	...
	<div class="col-md-12">
		<div class="portlet light portlet-fit portlet-form bordered">
			...
			<div class="portlet-body form-body">
				<form class="form-horizontal formIncome" method="POST" action="{{url('accounting/expense/insert')}}" id="form_validation">
					...
					<div class="form-body">
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-6">
							...
							<div class="form-group{{ $errors->has('expense_date') ? ' has-error' : '' }}  margin-top-20">
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<input class="form-control datepicker" type="text" name="expense_date" id="expense_date" placeholder="{{trans('template.date')}} - day-Month-Year" >
									</div>
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-6">
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
								...
								<label class="control-label col-md-5" id="AmountPayment" style="text-align: center;font-size: 18px;color: red;"> $ 0.00</label>	
								<input type="hidden" name="amount_have" id="amount_have" value="0">		    
							</div>
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
						...
						<table class="table table-striped table-bordered table-hover dt-responsive" id="tableExpense" width="100%" >
							<thead>
								...
								<tr>
									...
									<td style="width: 30%;">
										<select name="arr_chart_account[]" class="form-control select2 chart_account_name">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getAllChart(0,0,'',5)}}
										</select>
									</td>
									<td>
									...
									<td>
										<select name="tax[]" class="form-control select2 select_tax select_tax0" onchange="ClickTax(this,0)">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
...
$(document).ready(function() {
	table_income = $('#tableExpense').DataTable({
		"filter":   false,
		"paging":   false,
	    "ordering": false,
	    "info":     false
...
function CalculateDataTable(){
	var amount = 0;
	var amount_tax = 0;
	$('#tableExpense tbody tr').each(function() {
...
function onSubmit(){
	...
	if(referent_no=="" || expense_date=="" || user_id==0 || chart_account==0 || chart_account_name==0 || enter_price =="" || chart_account==0 || chart_account_name==0 || enter_price =="" ){
		if(referent_no !=""){
		...
		if(expense_date !=""){
			$('#expense_date').css("border", "1px solid #ccc");
		}else{
