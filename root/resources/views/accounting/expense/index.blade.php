...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<span>{{ trans('template.expense') }}</span>
		</li>
	</ul>
</div>
<br/>
		...
		<div class="portlet light bordered">			
			<div class="portlet-title">
				<div class="caption font-green">
					...
					<span class="caption-subject bold uppercase">{{ trans('template.expense') }}</span>
				</div>
				<div class="actions">
					<a title="{{ trans('template.add_new') }}" class="btn btn-circle btn-icon-only btn-default" href="{{ url('accounting/expense/add') }}">
			...
			<div class="portlet-body">
				...
				@endif
				<table class="table table-striped table-bordered" width="100%" id="table-expense">
					<thead>
						<tr class="bg-primary">
				...
				<script id="details-template" type="text/x-handlebars-template">
			        <table class="table details-table" id="expense-@{{id}}">
			            <thead>
			                <tr>
...
<script type="text/javascript">	
	...
	var table =$('#table-expense').DataTable({
		processing: true,
		serverSide: true,
		ajax: '{{url("/accounting/expense/list")}}',
		columns: [
			{
	...
	$('#table-expense tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = table.row(tr);
        var tableId = 'expense-' + row.data().id;       
        if (row.child.isShown()){
            ...
            $('#expense-'+row.data().id+'_length').addClass('col-xs-12 col-md-6');
        }
    });
    function initTable(tableId,id){
        ...
        $('#'+tableId).DataTable({
            ...
            ajax:'{{url("")}}/accounting/expense/getDetailData/'+id,
            columns:[
	...
	function onDelete(id){
		...
		App.doDelete("{{ trans('template.delete').' '.trans('template.expense') }}","{{trans('template.message_delete').' '.trans('template.expense').' ?'}}",'/accounting/expense/delete',id,'post',_token);
	}
</script>
@endsection()