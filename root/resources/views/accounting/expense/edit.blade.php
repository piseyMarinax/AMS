...
@section('content')
 <div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="{{url('accounting/expense/index')}}">​{{ trans('template.expense') }}</a>
			<i class="fa fa-circle"></i>
		</li>
		<li>
		...
		<div class="portlet light portlet-fit portlet-form bordered">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green sbold uppercase">{{ trans('template.expense') }}</a></span>
				</div>
			</div>
			<div class="portlet-body form-body">
				<form class="form-horizontal formIncome" method="POST" action="{{url('accounting/expense/update')}}" id="form_validation">
					<input type="hidden" name="_token" value="{{ csrf_token() }}" />
					<input type="hidden" name="update_id" value="{{$expense->jn_id}}" />
					<div class="form-body">
						@if(count($errors))
							...
							<div class="form-group{{ $errors->has('referent_no') ? ' has-error' : '' }}  margin-top-20">
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<i class="fa"></i>
										<input type="text" value="{{$expense->code}}" class="form-control" name="referent_no" id="referent_no" placeholder="{{trans('template.referent_no')}}" /> </div>
									@if($errors->has('referent_no'))
	                                        <span style="color:red">{!!$errors->first('referent_no')!!} </span>
									@endif	
								</div>
							</div>
							<div class="form-group{{ $errors->has('expense_date') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-3">{{trans('template.date')}}
								<span class="required"> * </span>
								</label>			                    
								<div class="col-md-9">
									<div class="input-icon right">
										<input class="form-control datepicker" type="text" value="<?php echo date('d-m-Y',strtotime($expense->expense_date)); ?>" name="expense_date" id="expense_date" placeholder="{{trans('template.date')}} - day-Month-Year" >
									</div>
									@if($errors->has('expense_date'))
	                                        <span style="color:red">{!!$errors->first('expense_date')!!} </span>
									@endif	
								</div>
							</div>
								...
								<div class="col-md-9">
									<div class="input-icon right">
										<select name="user_id" class="form-control select2" id="user_id" style="width: 100%;">
											...
											{{\App\Helpers\Helpers::getUsers($expense->user_id)}}
										</select>
									</div>
									@if($errors->has('name'))
						...
						<div style="border: 1px solid #b7b7b7; background: rgba(173, 203, 206, 0.28);    margin-top: -15px;" class="col-md-6">
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
								<label class="control-label col-md-5" style="color: green; font-size: 18px;">{{trans('template.direct_expense')}}</label>
								<label class="control-label col-md-5" id="AmountPayment" style="text-align: center;font-size: 18px;color: red;"> $ <?php echo number_format($amount->total_amount,2)?></label>	
								<input type="hidden" name="amount_have" id="amount_have" value="{{$amount->total_amount}}">		 
							</div>
							<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
								...
								<div class="col-md-8">
									<div class="input-icon right">
										<select name="chart_account" class="form-control select2" id="chart_account">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getChartAccountID([1,4,5],$expense->acc_id)}}
										</select>
									</div>
								...
								<div class="col-md-8">
									<div class="input-icon right">
										<select name="class" class="form-control select2 value_class" id="AddClass">
											...
											{{\App\Helpers\Helpers::getClasses(0,0,$expense->class)}}
										</select>
									</div>
									@if($errors->has('class'))
								...
								<tr>
									...
									<td style="width: 30%;">
										<select name="arr_chart_account[]" class="form-control select2 chart_account_name">
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getAllChart(0,0,$data->jn_ac_type,5)}}
										</select>
									</td>
									<td>
										<input type="text" value="<?php echo $data->jn_drb;?>" class="form-control enter_price enter_price<?php echo $key;?>" value="" onkeyup="EnterQty(this)" name="amount[]" placeholder="{{trans('template.amount')}}">
									</td>
									<td>
									...
									<td>
										...
										<select name="tax[]" class="form-control select2 select_tax select_tax<?php echo $key;?>" onchange="ClickTax(this,0)" disabled>
											<option value="0">--- {{trans('template.please_select')}} ---</option>
											{{\App\Helpers\Helpers::getTax($data->tax_id)}}
										</select>
										<?php 
											if($data->tax_id !=0){
												foreach($tax as $keytax=>$rowtax){ ?>
												<?php if($rowtax->id == $data->tax_id){ ?>
													<input type="hidden" value="<?php echo $rowtax->tax_vale; ?>" name="tax_value" class="value_tax value_tax<?php echo $key;?>">
												<?php }?>
										<?php  }
											}else{ ?>
												<input type="hidden" value="0" name="tax_value" class="value_tax value_tax<?php echo $key;?>">
										<?php }	?>
									</td>
								</tr>
								<?php } ?>
					...
					<div class="form-actions">
						<div class="row">
							<div class="col-md-offset-4 col-md-6">
								<button type="submit"  name="btnSave" value="save" class="btn green" id="Save"> <i class="fa fa-save"> </i> <strong> {{trans('template.update')}}</strong></button>
								<a href="{{url('accounting/expense/index')}}"><button type="button" id="btnClose" class="btn btn-danger"> <i class="fa fa-times-circle"> </i> <strong> {{trans('template.cancel')}}</strong></button> </a>
							</div>
						</div>
					</div>
				</form>
...
var counter = $('#tableIncome tr').length-1;
...
function GrandTotal(){
	...
	var grand_have = $('#amount_have').val();
	if(parseFloat(grand_have) >= parseFloat(sub_total)+parseFloat(total_tax)){		
...
$('#chart_account').change(function(){
	var id = $(this).val();
	$.get('{{url("/accounting/expense/calulate")}}/'+id,function(data){
		if(data.total_amount ==0 ||data.total_amount=="" || data.total_amount ==null ){
			$('#AmountPayment').html('$ 0.00');
			$('#Save').css('visibility','hidden');
			$('#amount_have').val(0);	
		}else{
			$('#AmountPayment').html('$ 0.00');
			$('#AmountPayment').html(formatCurrency(data.total_amount));
			$('#amount_have').val(0);
			$('#amount_have').val(data.total_amount);
			$('#Save').css('visibility','visible');
		}
	});
});
