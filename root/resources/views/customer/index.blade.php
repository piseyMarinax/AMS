...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<span>{{ trans('template.customer_list') }}</span>
		</li>
	</ul>
</div>
<br/>
		...
		<div class="portlet light bordered">
			<div class="portlet-title">
				<div class="caption font-green">
					...
					<span class="caption-subject bold uppercase">{{ trans('template.customer_info') }}</span>
				</div>
				<div class="actions">
					<a title="{{ trans('template.add_new') }}" class="btn btn-circle btn-icon-only btn-default" href="{{ url('/customer/add') }}">
...
<script type="text/javascript">
	$('#users-table').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: '{{url("customer/jsonCustomers")}}',
		columns: [
			{data: 'cus_code'},
			{data: 'cus_name'},
			{data: 'address'},
	...
	function onDelete(id){
		...
		App.doDelete("{{ trans('template.delete').' '.trans('template.customer') }}","{{trans('template.message_delete').' '.trans('template.customer').' ?'}}",'/customer/delete',id,'post',_token);
	}
</script>
@endsection()