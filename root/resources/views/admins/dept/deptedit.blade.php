...
@section('content')
 <div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="{{url('/admins/dept/index')}}">{{ trans('template.department') }}</a>
			<i class="fa fa-circle"></i>
		</li>
		<li>
		...
		<div class="portlet light portlet-fit portlet-form bordered">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green sbold uppercase">{{ trans('template.department') }}</a></span>
				</div>
			</div>
			<div class="portlet-body form-body">
				<form class="form-horizontal" method="POST" action="{{url('/admins/dept/update')}}" id="form_validation">
					...
					<div class="form-body">
						...
						<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}  margin-top-20">
							...
							<div class="col-md-6">
								<div class="input-icon right">
									<i class="fa"></i>
									<input type="text" class="form-control" value="{{$edit->dept}}" name="name" id="name" /> </div>
									@if($errors->has('name'))
						...
						<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}  margin-top-20">
							...
							<div class="col-md-6">
								<div class="input-icon right">
									<i class="fa"></i>
									<input type="text" value="{{$edit->ddetail}}" class="form-control" name="des" id="description" /> </div>
					...
					<div class="form-actions">
						<div class="row">
							<div class="col-md-offset-4 col-md-6">
								<button type="submit"  name="btnSave" value="save" class="btn green"> <i class="fa fa-edit"> </i> <strong> {{trans('template.save_change')}}</strong></button>
								<a href="{{url('admins/dept/add')}}"><button type="button" id="btnClose" class="btn btn-danger"> <i class="fa fa-times-circle"> </i> <strong> {{trans('template.cancel')}}</strong></button> </a>
							</div>
						</div>
					</div>
				</form>
						...
						<table class="table table-striped table-bordered" width="100%" id="table-dept">
							<thead>
								<tr class="bg-primary">
									...
									<th class="text-center min-phone-l">{{trans('template.department')}}</th>
...
<script type="text/javascript">
	$('#table-dept').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: '{{url("admins/dept/getJsonDept")}}',
		columns: [
			{data: 'rownum'},
			{data: 'dept'},
			{data: 'dstatus'},
	...
	function onDelete(id){
		...
		App.doDelete("{{ trans('template.delete').' '.trans('template.department') }}","{{trans('template.message_delete').' '.trans('template.department').' ?'}}",'/admins/dept/delete',id,'post',_token);
	}
</script>
@endsection()