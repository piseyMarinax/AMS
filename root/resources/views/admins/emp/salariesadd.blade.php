...
@section('content')
 <div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="{{ url('/admins/staff/index') }}">{{ trans('template.salary') }}</a>
			<i class="fa fa-circle"></i>
		</li>
		<li>
			<span>{{ trans('template.add_new') }} {{ trans('template.salary') }}</span>
		</li>
	</ul>
</div>
<br/>
	...
	<div class="col-md-12">
		<div class="portlet box blue">
			<div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-money"></i> <strong>{{ trans('template.salary') }}</strong> </div>
                <div class="tools">
			...
			<div class="portlet-body">
				<form class="form-horizontal" role="form" method="POST" action="{{ url('admins/staff/stsalariesSave') }}" enctype="multipart/form-data">
					{{ csrf_field() }}
					<div class="form-body">
						@if(count($errors))
						...
						<hr>
							<div class="col-md-6">
								<div class="form-group{{ $errors->has('salamount') ? ' has-error' : '' }}">
									<label for="salamount" class="col-md-4 control-label"><strong>{{ trans('template.salary') }} ($)</strong>
										<span class="required"> </span>
									</label>
									<div class="col-md-8">
										<input id="salamount" type="number" class="form-control" name="salamount" value="" autofocus>
										@if ($errors->has('salamount'))
											<span class="help-block">
												<strong>{{ $errors->first('salamount') }}</strong>
											</span>
										@endif
									</div>
								</div>
							...
							<div class="col-md-6">
								<div class="form-group{{ $errors->has('startingdate') ? ' has-error' : '' }}">
									<label for="startingdate" class="control-label col-md-4"><strong>{{trans('template.startingdate')}}</strong>
										<span class="required">  </span>
									</label>			                    
									<div class="col-md-8">
										<div class="input-icon right">
											<input class="form-control datepicker" type="text" name="startingdate" id="startingdate" placeholder="{{trans('template.date')}}: dd-mm-yyyy" required>
										</div>
										@if($errors->has('startingdate'))
		                                        <span style="color:red">{!!$errors->first('startingdate')!!} </span>
										@endif	
									</div>
								</div>
								
								<div class="form-group{{ $errors->has('expireddate') ? ' has-error' : '' }}">
									<label for="expireddate" class="control-label col-md-4"><strong>{{trans('template.expireddate')}}</strong>
										<span class="required"> </span>
									</label>			                    
									<div class="col-md-8">
										<div class="input-icon right">
										...
										@if($errors->has('expireddate'))
		                                        <span style="color:red">{!!$errors->first('expireddate')!!} </span>
										@endif	
									</div>
								</div><!--
