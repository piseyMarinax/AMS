...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<span>{{ trans('template.absentt') }}</span>
		</li>
	</ul>
</div>
<br/>
					...
					<ul class="nav nav-tabs">
						...
						<li>
							<a href="#tab_1_2" data-toggle="tab"> 
								<i class="fa fa-table font-green"></i>
								<span class="caption-subject bold uppercase">{{ trans('template.absentt_type') }}</span>
							</a>
						</li>
					</ul>
					<div class="tab-content">
						<div class="tab-pane fade active in" id="tab_1_1">
							<div class="actions" style="float:right;padding:8px;">
								<a title="{{ trans('template.add_new') }}" class="btn btn-circle btn-icon-only btn-default" href="{{ url('admins/absentt/add') }}">
							...
							<div class="portlet-body">
								...
								<table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="table-ar">
									<thead>
										<tr class="bg-primary">
											<th></th>
											<th>{{ trans('template.referent_no') }}</th>
											<th>{{ trans('template.customer') }}</th>
											<th>{{ trans('template.amount') }}</th>
											<th>{{ trans('template.date') }}</th>
											<th>{{ trans('template.name') }}</th>
											<th>{{ trans('template.status') }}</th>
											<th>{{ trans('template.remaining_balance') }}</th>							
