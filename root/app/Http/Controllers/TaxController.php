...
class TaxController extends Controller
{
   public function __construct(){
	...
	public function getIndex(){
		return view('setting.tax.index');
	}
	public function getJsonList(){
    	$row = TableTax::select()->where('status',1);
	...
	public function insertData(Request $data){
        ...
        if($validation->fails()){
        	...
        	if($data->id_update !=0){
        		TableTax::where('id',$data->id_update)->update([
	            ...
	            return response()->json(['row'=>TableTax::find($data->id_update),'validation'=> $validation]);
        	}else{
	            $id = TableTax::insertGetId([
	                'tax_name'=>$data->tax_name,
	                'tax_vale'=>$data->tax_value,
	                'status'=>1,
	                'tax_detail'=>$data->tax_des,
	            ...
	            return response()->json(['row'=>TableTax::find($id),'validation'=> $validation]);
        	}
        }
	}
	public function getEdit($id){
		return response()->json(TableTax::find($id));
	}
	public function getDelete(Request $data){
        if($data->ajax() && TableTax::find($data['id'])){
            TableTax::where('id',$data['id'])->delete();
        }        
    }
}
